...
- **스와이프 삭제 제스처 자체**: `Gesture.Pan` 리플레이가 시뮬레이터 스크립트 터치로는 인식되지 않음(같은 `GestureDetector`의 `LongPress`는 정상 동작해 배선 자체는 문제없음 — Pan 특유의 연속 이동 이벤트가 스크립트 터치로 재현이 잘 안 되는 시뮬레이터 자동화 한계로 추정). **실기기/수동 확인 필요.**

## 남은 작업

P7 종료 — 다음은 P8 (W21–24) 출시: EAS 빌드, iOS/Android 스토어 제출, Sentry, OTA.
