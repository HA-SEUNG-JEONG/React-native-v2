...
- [ ] `recyclingKey`가 없으면 왜 잔상이 생기는지 (FlatList 재활용) 설명 가능한가?
- [ ] `cachePolicy` 4값의 차이와 언제 `memory-disk`를 쓰는가?
- [ ] `useMutation`의 onMutate → mutationFn → onError/onSuccess → onSettled 순서를 그릴 수 있나?
- [ ] onMutate에서 `cancelQueries`를 먼저 부르는 이유는?
- [ ] 낙관적 업데이트에서 `invalidateQueries`가 아니라 `setQueryData`를 쓰는 이유는?
- [ ] invalidate 범위를 좁혀야 하는 이유 (PR #59 버그) 를 코드로 재현할 수 있나?
- [ ] 두 화면이 같은 `queryKey`를 쓰면 요청이 몇 번 나가는가?
- [ ] FlatList `data`와 SectionList `sections`의 모양 차이는?
