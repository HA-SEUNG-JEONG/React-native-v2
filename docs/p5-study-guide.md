...
`canceled`는 **권한은 있는데 유저가 갤러리에서 그냥 뒤로 나온** 경우다. 거부가 아니다. 여기서 `setUri(null)`을 하면 이전에 고른 사진이 이유 없이 사라진다. 아무것도 안 하고 물러나는 게 맞다.
...
예전 코드는 `ImagePicker.MediaTypeOptions.Images` 열거형을 썼다. SDK 54는 문자열(`"images"` / `"videos"` / `"livePhotos"`)을 받고 배열도 가능하다. 인터넷 예제 대부분이 구버전이라 그대로 베끼면 deprecation 경고가 뜬다. **버전 고정 문서를 봐야 하는 이유** (`AGENTS.md` 규칙).
...
2. `style`을 아예 비워뒀다 — `expo-image`는 크기를 안 주면 **0×0**이라 아무것도 안 보인다. 웹 `<img>`처럼 원본 크기로 알아서 커지지 않는다.
...
> 참고: iOS에서 갤러리 **읽기**는 상황에 따라 권한 다이얼로그를 안 띄우기도 한다. 그래서 "다이얼로그가 안 뜨네 = 코드 버그"로 오해하기 쉽다. 수동으로 권한을 끄고 테스트하는 게 확실하다.
...
- **W12** 위치/지도 — `expo-location`. 같은 권한 플로우 + "사용 중에만 허용" 같은 부분 권한 개념 추가.
- **W13** 저장/보안 — `expo-secure-store`, `AsyncStorage`. 권한은 없지만 저장 실패 처리가 관건.
- **W14** 알림/공유 — `expo-notifications`. 권한 + 백그라운드 동작.
