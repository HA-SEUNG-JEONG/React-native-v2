...
### 카메라 — 배너가 처음으로 진짜 값을 하는 지점

> 문서: [`launchCameraAsync`](https://docs.expo.dev/versions/v54.0.0/sdk/imagepicker/#imagepickerlaunchcameraasync) · [`useCameraPermissions`](https://docs.expo.dev/versions/v54.0.0/sdk/imagepicker/#imagepickerusecamerapermissions)

갤러리(`launchImageLibraryAsync`)는 권한이 필요 없었다. 카메라는 **다르다.** 문서상 `launchCameraAsync`는 `Permissions.CAMERA`를 실제로 요구한다. 그래서 위에서 만든 거부 UX 배너가 여기서 처음으로 시뮬레이션이 아니라 진짜로 작동한다.
...
1. 게이트에서 카메라 훅이 아니라 미디어 훅(`requestPermission`)을 요청 — 카메라 권한을 물어야 하는데 갤러리 권한을 물었다. `cameraRequestPermission`로 수정.
2. `if (!result.canceled) return;` — 성공에서 return하고 취소에서 `assets[0]` 널참조로 크래시. `if (result.canceled) return;`이 맞다(함정 4와 동일한 실수).

### `cameraPermission`은 생략해도 된다 (문서 확인)
